...
import ${modulePackage}.${moduleName}.controller.${modelNameUpperCamel}Controller;
...
public class ${modelNameUpperCamel}ServiceImpl extends AbstractService<${modelNameUpperCamel}> implements ${modelNameUpperCamel}Service {
    ...
    public Boolean isValid(String key, String value) {
        ...
        ${modelNameUpperCamel}Controller.VALID_MESSAGE.forEach((k, v) -> {
            if (StringUtils.equals(k, key)) {
                criteria.andEqualTo(key, value);
            }
        });

        int count = ${modelNameLowerCamel}Mapper.selectCountByExample(example);
        if (count == 0) {
            return true;
        }
        return false;
